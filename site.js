...
if (tuple) {
  tuple.addEventListener("click", (event) => {
    const button = event.target.closest("button[data-field]");
    if (!button) return;
    const field = fields[button.dataset.field];
    tuple.querySelectorAll("button").forEach((item) => item.classList.remove("is-active"));
    button.classList.add("is-active");
    document.querySelector("[data-field-count]").textContent = `${field[0]} / 09`;
    document.querySelector("[data-field-key]").textContent = button.dataset.field;
    document.querySelector("[data-field-title]").textContent = field[1];
    document.querySelector("[data-field-copy]").textContent = field[2];
    document.querySelector("[data-field-example]").textContent = field[3];
  });
}
